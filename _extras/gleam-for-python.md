...
*Taken from [this page](https://gleam.run/cheatsheets/gleam-for-python-users/).
...
A docstring that occurs as the first statement in a module, function, class, or method definition will become the `__doc__`attr of that object.
...
## Operators
  ...
  Remainder            `%`      `%`     In Gleam both values must be **ints**, in Gleam negative values behave differently: Use `int.modulo`to mimic Python's behavior.
